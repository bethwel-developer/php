...
const handleDelete = (id)=>{
  ...
  return (
<div class="flex justify-center align-center overflow-x-auto ">
    <div>

    <h2 className='font-bold text-2xl  text-center  mb-20 mt-8'>MYSQL REACT CRUD APPLICATION </h2>
    ...
    <table class=" text-sm text-left text-gray-500 dark:text-gray-400">
        ...
        {users.map((n, i)=> (
            ...
            <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700"  key={i}>
                ...
                <td class="font-medium text-black px-6 py-4">
                {n.name}
                </td>
                <td class=" font-medium text-black px-6 py-4">
                {n.email}
...
<button className='focus:outline-none mr-4  text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2 mr-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800' >
                              <Link to={`/update/${n.id}`}>edit</Link></button>
                </td>
            </tr>
               ))}
        </tbody>
